...
class DomainRegistry:
    ...
    def register_domain(self, domain: BaseDomainModule) -> None:
        ...
        if config.domain_name in self._domains:
            ...
            # Add to load order if new domain
            self._load_order.append(config.domain_name)
    ...
    def auto_load_domains(
        ...
        Args:
            ...
            base_path: Base path for domain modules (defaults to gpdisc_core.domains)

        Returns:
            Dictionary mapping domain names to load success status
        """
        if base_path is None:
            base_path = "gpdisc_core.domains"
    ...
    def _load_domain_module(
        ...
        try:
            ...
            # Try to use factory function first (create_XXX_domain pattern)
            factory_name = f"create_{domain_name}"
            if hasattr(module, factory_name):
                create_func = getattr(module, factory_name)
                domain_instance = create_func()
            else:
                # Fall back to direct instantiation
                domain_instance = domain_class()

            # Initialize domain
            if hasattr(domain_instance, 'initialize'):
                try:
                    domain_instance.initialize(self._global_config)
                except:
                    pass  # Initialize may not be implemented
