"""
Enhanced unified GPDISC system with all Phase 2-4 enhancements
...
This is the main entry point for the enhanced GPDISC system.
"""
...
try:
    from .unified import UnifiedGPDISCSystem, UnifiedConfig, TaskType, TaskAnalyzer
...
# Import clinical reasoning core (GP-led front door, expertise program Stage 1)
try:
    from gpdisc_core.clinical_reasoning import ConsultationPipeline
except ImportError:
    ConsultationPipeline = None
    logger.warning("Clinical reasoning core not available")
...
class EnhancedUnifiedGPDISCSystem:
    """
    Enhanced unified GPDISC system with all Phase 2-4 capabilities
    ...
    def __init__(self, config: Optional[EnhancedUnifiedConfig] = None):
        ...
        # Initialize clinical reasoning pipeline (GP-led front door)
        ...
        if UnifiedGPDISCSystem is not None:
            ...
            self.base_system = UnifiedGPDISCSystem(config=base_config)
        ...
        logger.info("EnhancedUnifiedGPDISCSystem initialized")
    ...
    def process_query(
        ...
        This is the main entry point for interacting with GPDISC.
    ...
    def answer(self, query: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        # Additive enrichment: never replaces or breaks the existing answer path.
        if self.consultation_pipeline is not None:
            try:
                ...
                result["consultation"] = {
                    ...
                    "syndrome_differentials": rec.syndrome_differentials,
                ...
                # The front door: the validated consultation IS the answer.
                # The legacy domain text moves aside to metadata, never away
                # (2026-09-03 global audit: the diagnosis was computed and
                # then hidden behind a system-init banner).
                result["legacy_answer"] = result.get("answer", "")
                result["answer"] = rec.summary()
            except Exception as exc:
                # Reasoning core must never break the consultation system
                logger.debug(f"Clinical reasoning enrichment skipped: {exc}")
        return result
    ...
    def _process_with_counterfactual(
        ...
        This is the key method that enables GPDISC's "nascent discovery mode"—the
...
def create_gpdisc_system(config: Optional[EnhancedUnifiedConfig] = None) -> EnhancedUnifiedGPDISCSystem:
    """
    Factory function to create GPDISC system

    Args:
        config: Optional configuration

    Returns:
        EnhancedUnifiedGPDISCSystem instance
    """
    return EnhancedUnifiedGPDISCSystem(config)


# Backwards compatibility aliases
create_enhanced_stan_system = create_gpdisc_system  # Deprecated
create_stan_system = create_gpdisc_system  # Deprecated
