#!/usr/bin/env python3
"""
Comprehensive Integration Check for GPDISC
...
# Bootstrap: add repo root (parent of gpdisc_core) to path for direct execution
sys.path.insert(0, str(Path(__file__).parent.parent.parent))

warnings.filterwarnings('ignore')

print("=" * 80)
print("BIODISC COMPREHENSIVE INTEGRATION CHECK")
print("=" * 80)
print()
...
def test_core_imports():
    ...
    modules = [
        'gpdisc_core',
        'gpdisc_core.reasoning',
        'gpdisc_core.physics',
        'gpdisc_core.domains',
        'gpdisc_core.causal',
        'gpdisc_core.memory',
        'gpdisc_core.intelligence',
        'gpdisc_core.capabilities',
    ]
...
def test_v47_causal_imports():
    ...
    from gpdisc_core.causal.inference import (
    ...
    from gpdisc_core.causal import (
        BayesianStructureLearner,
        ExpectedInformationGainCalculator,
        OnlineCausalLearner,
        SimulationBasedInferenceEngine,
...
def test_v47_domain_imports():
    ...
    from gpdisc_core.domains import (
    ...
    print("  ✓ MolecularBiologyDomain imported")
    print("  ✓ BiochemistryDomain imported")
    print("  ✓ GeneticsDomain imported")

    # Test domain creation
    mb_domain = create_molecular_biology_domain()
    print(f"  ✓ MolecularBiologyDomain created: {len(mb_domain.get_capabilities())} capabilities")

    bc_domain = create_biochemistry_domain()
    print(f"  ✓ BiochemistryDomain created: {len(bc_domain.get_capabilities())} capabilities")

    g_domain = create_genetics_domain()
    print(f"  ✓ GeneticsDomain created: {len(g_domain.get_capabilities())} capabilities")
...
def test_v47_metalearning_imports():
    ...
    try:
        from gpdisc_core.reasoning.maml_optimizer import (
            MAMLOptimizer,
...
def test_physics_cross_module_calls():
    ...
    from gpdisc_core.physics import UnifiedPhysicsEngine, RelativisticPhysics
    from gpdisc_core.domains import BaseDomainModule, DomainConfig
...
def test_causal_physics_integration():
    ...
    from gpdisc_core.causal import StructuralCausalModel
    from gpdisc_core.physics import PhysicsConstraint
...
def test_domain_causal_integration():
    ...
    from gpdisc_core.domains import MolecularBiologyDomain

    domain = MolecularBiologyDomain()
    # Test that domain can process queries
    result = domain.process_query("How does DNA replication work?", context={})
    print(f"  ✓ MolecularBiologyDomain.process_query works: confidence = {result.confidence:.2f}")
...
def test_metalearning_domain_integration():
    ...
    try:
        from gpdisc_core.reasoning.maml_optimizer import MAMLOptimizer
...
def test_domain_orchestrator():
    ...
    domains_to_register = [
        ('molecular_biology', lambda: __import__('gpdisc_core.domains.molecular_biology', fromlist=['create_molecular_biology_domain']).create_molecular_biology_domain()),
        ('biochemistry', lambda: __import__('gpdisc_core.domains.biochemistry', fromlist=['create_biochemistry_domain']).create_biochemistry_domain()),
        ('genetics', lambda: __import__('gpdisc_core.domains.genetics', fromlist=['create_genetics_domain']).create_genetics_domain()),
    ]
...
def test_unified_orchestrator():
    ...
    try:
        from gpdisc_core.core.unified import create_unified_stan_system
        print("  ✓ create_unified_stan_system imported")
    except ImportError as e:
        print(f"  ⚠ Unified system not available (expected in degraded mode): {e}")

    try:
        from gpdisc_core.core.unified_enhanced import create_enhanced_stan_system
...
def test_causal_graceful_degradation():
    ...
    from gpdisc_core.causal.discovery.bayesian_structure_learning import (
        BayesianStructureLearner,
        InferenceMethod,
    )
...
def test_domains_graceful_degradation():
    ...
    for domain_class, create_fn, name in [
        (MolecularBiologyDomain, None, "MolecularBiologyDomain"),
        (BiochemistryDomain, None, "BiochemistryDomain"),
        (GeneticsDomain, None, "GeneticsDomain"),
    ]:
        try:
            domain = domain_class()
...
def test_active_discovery_pipeline():
    ...
    from gpdisc_core.causal.discovery.eig_calculator import (
        create_eig_calculator, ObservationPlan, NoiseModel
    )
    from gpdisc_core.causal.discovery.online_causal_learning import (
...
def run_all_tests():
    ...
    if failed == 0:
        ...
        print("\nBIODISC is fully integrated with:")
